    ...
    (let ((bg-back   '(:background back))
      ...
      (mapcar (lambda (face) (apply 'create-face-spec face))
              `(;; basic
				...
				(org-table (,@fg-blue))
				(org-column (,@bg-base02))
				(org-column-title (,@fmt-undr ,@fmt-bold ,@bg-base02))
				(org-scheduled (,@fg-green))
				(org-scheduled-today (,@fg-green))
				(org-scheduled-previously (,@fg-orange))
				(org-date (,@fg-violet))
				(org-sexp-date (,@fg-violet))
				(org-date-selected (,@fmt-bold ,@fg-red))
				(org-document-info (,@fg-blue))
				(org-document-title (,@fmt-bold ,@fg-blue))
				(org-drawer (,@fg-blue))
				(org-footnote (,@fmt-undr ,@fg-violet))
				(org-formula (,@fg-orange))
				(org-latex-and-related (,@fg-orange))
				(org-macro (,@fg-orange))
				(org-upcoming-deadline (,@fg-red))
				(org-agenda-structure (,@fg-blue))
				(org-agenda-done (,@fg-green))
				(org-agenda-restriction-lock (,@bg-base02))
